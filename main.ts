...
function initNebula () {
    ...
    for (let y = 0; y <= 4; y++) {
        ...
        for (let x = 0; x <= 4; x++) {
            sprite3 = game.createSprite(x, y)
            sprite3.set(LedSpriteProperty.Brightness, 0)
            sprite3.set(LedSpriteProperty.Direction, 180)
            row.push(sprite3)
        }
        nebula.push(row)
    }
...
let temp_star: game.LedSprite = null
let menuIndex = 0
...
let soundOn = false
let background_nebula: game.LedSprite[][] = []
